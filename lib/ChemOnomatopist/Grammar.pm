...
my @rules = (
    ...
    # S-based groups
    [ sub { &is_nongroup_atom && &is_S }, \&is_ketone, \&is_hydroxy, \&is_C, NO_MORE_VERTICES,
      sub { graph_replace( $_[0], ChemOnomatopist::Group::SulfinicAcid->new( @_[2..3] ), @_[1..3] ) } ],
    [ sub { &is_nongroup_atom && &is_S }, ( \&is_ketone ) x 2, \&is_hydroxy, \&is_C, NO_MORE_VERTICES,
    ...
    [ sub { &is_nongroup_atom && &is_O_S_Se_Te }, sub { &is_nongroup_atom && &is_O && &charge_minus_one }, \&is_C,
      sub { graph_replace( $_[0], ChemOnomatopist::Group::Hydroperoxide->new( $_[1], $_[2] ), @_[1..2] ) } ],
