...
sub nonstandard_valence_positions()
{
    ...
    for (0..$#vertices) {
        ...
        next if ChemOnomatopist::Util::element( $vertices[$_] ) eq 'N';
...
sub max_valence()
{
    ...
    my $max_valence = 0;
    for my $vertex ($self->vertices) {
        my $element = ChemOnomatopist::Util::element( $vertex );
        next unless $element;
...
sub needs_heteroatom_names()
{
    ...
    if( @vertices == 1 && $self->parent &&
        grep { ChemOnomatopist::Util::element( @vertices ) eq $_ } qw( S Se Te ) ) {
        return '';
    }
...
sub needs_suffix_locant()
{
    ...
    return 1 if !ChemOnomatopist::Util::element( $most_senior_groups[0] );
    return 1 if  ChemOnomatopist::Util::element( $most_senior_groups[0] ) ne 'C';
    return '';
}
...
sub heteroatoms()
{
    my( $self ) = @_;
    my @vertices = $self->vertices;
    return map { ChemOnomatopist::Util::element( $vertices[$_] ) }
...
sub isotopes()
{
    ...
    for my $i (0..$#vertices) {
        ...
        if( exists $vertices[$i]->{isotope} ) {
            push @isotopes, ChemOnomatopist::Isotope->new( ChemOnomatopist::Util::element( $vertices[$i] ),
                                                           $vertices[$i]->{isotope},
                                                           $i,
                                                           $self->locants( $i ) );
        }
...
sub stereodescriptor_part()
{
    ...
    my @vertices = $self->vertices;
    my $graph = $self->graph;
    my $double_bond_positions = set( $self->double_bond_positions );
    ...
    for my $i (0..$#vertices) {
        ...
        if( is_chiral_tetrahedral( $vertices[$i] ) ) {
            ...
            next if $i == $#vertices;
            next if any { blessed $_ } map { $vertices[$_] } ( $i, $i+1 );
...
sub suffix()
{
    ...
    my $name = ChemOnomatopist::Name->new;
    if( $self->length == 1 && !blessed $chain[0] &&
        ChemOnomatopist::Util::element( @chain ) ne 'C' ) {
    ...
    # BBv2 P-63.2.2.2
    if( $self->parent && @chain && (all { !blessed $_ } @chain) &&
        ChemOnomatopist::Util::element( @chain ) eq 'O' &&
        !@double && !@triple && all { ChemOnomatopist::Util::element( $_ ) eq 'C' } @chain[1..$#chain] ) {
    ...
    if( $self->isa( ChemOnomatopist::Chain::Amide:: ) ||
        ...
        scalar( uniq map { ChemOnomatopist::Util::element( $_ ) } @chain ) > 1 ) {
